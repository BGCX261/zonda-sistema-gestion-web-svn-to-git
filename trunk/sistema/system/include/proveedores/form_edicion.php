<?php
    ...
    if (isset($_REQUEST["codigo"])) {
        $query = "SELECT * FROM proveedores WHERE codigo = ".$_REQUEST["codigo"];
    ...
    if (isset($_REQUEST["type"])) {
        if ($_REQUEST['type'] == 'alta' || $_REQUEST['type'] == 'clonar') {
            $query = "SELECT MAX(codigo) + 1 FROM proveedores";
            $result = mysql_query($query);
...
<h1><?php
    if (isset($_REQUEST['type'])) {
        ...
        elseif ($_REQUEST['type'] == 'clonar') {
            print "Clonaci&oacute;n de proveedor";
        }
        else {
            print "Edici&oacute;n de proveedor";
        }
    }
    else {
        print "Edici&oacute;n de proveedor";
