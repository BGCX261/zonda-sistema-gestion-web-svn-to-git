    ...
    <tbody>
        ...
        <?php
            ...
            while ($Row = mysql_fetch_array($Result)) {
                ...
                if ($_SESSION["venta"]->articulo_en_venta($Row[0])) {
                    $art = $_SESSION["venta"]->traer_articulo($Row[0]);
                    print $art['cantidad'];
                }
                else {
                    print '0';
                ...
                if ($_SESSION["venta"]->articulo_en_venta($Row[0])) {
                    print '<form style="display: inline;">';
                    print_form_vars();
                    print '<input type="hidden" name="eliminar_articulo" value="'.$Row[0].'" />';
...
<?php
    ...
    function print_form_vars() {
        print '<input type="hidden" name="include" value="ventas" />';
        print '<input type="hidden" name="form" value="form_venta" />';
